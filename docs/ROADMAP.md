...
## Этап 3.6 — Полировка после аудита

- [x] OG-изображения (`/opengraph-image`, по одному на пост) — `next/og`, статически
      генерируются на билде
- [x] RSS-фид (`/rss.xml`) — автообнаружение через `<link rel=alternate>`, ссылка в футере
- [x] Аналитика (`app/components/Analytics.tsx`) — выключена по умолчанию, включается
      `NEXT_PUBLIC_PLAUSIBLE_DOMAIN` на билде; провайдер — не окончательное решение
- [x] Security review кода — находок не обнаружено (весь новый код работает только с
      контентом из репозитория на билде, рантайм-ввода нет вообще из-за `output: "export"`)
- [x] E2e-тесты (Playwright, `e2e/`) — навигация, блог/CTA, мобильное меню, 404,
      SEO-артефакты; гоняются против реального `npm run build` + `npm run start`, не
      `next dev`; обязательный шаг в CI
- [x] Единый `focus-visible` для клавиатурной навигации (акцентный цвет вместо
      дефолтного браузерного)
